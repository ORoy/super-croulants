...
// constants) — it's read once per mount, not tracked as a dependency.
export function useSheetBatch(ranges: BatchRange[]): void {
  const apiKey = import.meta.env.VITE_GOOGLE_SHEETS_API_KEY;

  useEffect(() => {
    const misses = ranges
      .map(r => ({ ...r, key: makeKey(r.kind ?? "raw", r.sheetName, r.range) }))
      .filter(r => claimForBatch(r.key));

    if (misses.length === 0) return;

    fetchSheetValuesBatch(misses, apiKey)
      .then(results => {
